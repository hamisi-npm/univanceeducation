...
export function ProgramHero({ content }: ProgramHeroProps) {
  return (
    <section
      ...
      className="relative -mt-14 overflow-hidden bg-brand-navy pt-14 text-white"
    >
      {content.image?.src ? (
        <Image
      ...
      <Container className="relative py-16 sm:py-20 lg:py-24">
        <div className={sectionStyles.header}>
          ...
          <p className={sectionStyles.descriptionOnDark}>{content.description}</p>
        </div>
      </Container>
    </section>
  );
        ...
        <dl className="grid grid-cols-2 gap-6 lg:grid-cols-4">
          {statistics.map((stat) => (
            <div key={`${stat.value}-${stat.label}`} className="space-y-1 text-center">
              <dt className="font-mono text-2xl font-semibold text-brand-navy sm:text-3xl">
                {stat.value}
              </dt>
